...
var _ = Describe("test ensureHostTunnelAddress", func() {
	...
	It("should be able to load Client config from Env", func() {
		Expect(err).NotTo(HaveOccurred())
	})

	c, err := client.New(*cfg)
	It("should be able to create a new client", func() {
		Expect(err).NotTo(HaveOccurred())
	})

	It("should be possible to create a backend client and clear out the backend", func() {
	...
	Describe("Test variations on how node names are detected.", func() {
		...
		_, ip4net, _ := net.ParseCIDR("192.168.0.1/24")
		ensureHostTunnelAddress(ctx, c, node.Name, []net.IPNet{*ip4net})
		n, err := c.Nodes().Get(ctx, node.Name, options.GetOptions{})
		It("should be able to successfully allocate an IPIP tunnel address", func() {
			Expect(err).NotTo(HaveOccurred())
			Expect(n.Spec.BGP.IPv4IPIPTunnelAddr).ToNot(BeNil())
		})
