...
| `IptablesBackend`                    | `FELIX_IPTABLESBACKEND`                    | This parameter controls which variant of iptables binary Felix uses. `Auto` can be set to have Felix attempt to auto detect the backend to use. If using Felix on a system that uses the netfilter-backed iptables binaries, set this to `NFT`. [Default: `Legacy`]  | `Auto`, `Legacy`, `NFT` |
